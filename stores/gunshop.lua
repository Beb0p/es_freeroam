...
Citizen.CreateThread( function()
	while true do
		...
		for i,b in ipairs(gunshop_locations) do
			if IsPlayerWantedLevelGreater(GetPlayerIndex(),0) == false and  gshp.opened == false and IsPedInAnyVehicle(LocalPed(), true) == false and GetDistanceBetweenCoords(b[1],b[2],b[3],GetEntityCoords(LocalPed())) < 2 then
		...
		if gshp.opened then
			...
			drawMenuTitle(menu.title, gshp.menu.x,gshp.menu.y + 0.08)
				...
				if i >= gshp.menu.from and i <= gshp.menu.to then
					...
					if button.costs ~= nil then
						if gshp.currentmenu == "weapons" then
							if HasPedGotWeapon(LocalPed(),GetHashKey(button.model),false) then
								drawMenuRight("OWNED",gshp.menu.x,y,selected)
							else
								drawMenuRight(button.costs.."$",gshp.menu.x,y,selected)
							end
						elseif gshp.currentmenu == "selected_weapon" then
							if Citizen.InvokeNative(0xDC16122C7A20C933,LocalPed(),GetHashKey(button.model),Citizen.PointerValueInt()) == GetAmmoInPedWeapon(LocalPed(),GetHashKey(button.model)) then
								drawMenuRight("FULL",gshp.menu.x,y,selected)
							else
								drawMenuRight(button.costs.."$",gshp.menu.x,y,selected)
							end
						else
